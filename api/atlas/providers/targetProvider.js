...
function isTargetProductUrl(value = "") {
  if (!isTargetUrl(value)) return false;
  ...
  try {
    ...
    return (
      ...
      /\/-\/A-\d+/i.test(url.pathname) ||
      /[?&]preselect=\d+/i.test(url.search)
    );
  } catch {
    return false;
  }
...
function normalizeTargetImageUrl(value = "") {
...
function isLikelyTargetImage(value = "") {
  const url = normalizeTargetImageUrl(value);
...
function pushImage(bucket, candidate = {}) {
  const url = normalizeTargetImageUrl(candidate.url);

  if (!isLikelyTargetImage(url)) return;
  ...
  bucket.push({
    ...
    alt: candidate.alt || "",
    source: candidate.source || "target",
    role: candidate.role || "",
    identity,
  });
}
    ...
    while ((match = pattern.exec(html))) {
      ...
      walkObject(parsed, (node) => {
        for (const [key, value] of Object.entries(node)) {
          ...
          if (
            typeof value === "string" &&
            /image|media|asset|thumbnail|zoom|scene7|primary/.test(
              normalizedKey
            )
          ) {
            pushImage(bucket, {
...
function extractScene7Urls(html, bucket) {
  const regex =
    /https:\\?\/\\?\/(?:target\.)?scene7\.com\/[^"'\\\s<>]+?\.(?:jpe?g|png|webp|avif)(?:\?[^"'\\\s<>]*)?/gi;
...
export function canHandleTargetUrl(url = "") {
  return isTargetProductUrl(url);
}

export function extractTargetGallery({
  html = "",
  url = "",
  maximum = MAX_IMAGES,
} = {}) {
  if (!canHandleTargetUrl(url)) {
    return {
      ...
      count: 0,
      error:
        "La URL no corresponde a un producto válido de Target.",
    };
  }
  ...
  if (!html) {
    return {
      ok: false,
      provider: "target",
      tcin: extractTcin(url),
      images: [],
      count: 0,
      error:
        "No se recibió el HTML del producto de Target.",
    };
  }
  ...
  return {
    ...
    provider: "target",
    tcin: extractTcin(url) || extractTcin(html),
    url,
    images: selected,
    ...
    error:
      selected.length > 0
        ? ""
        : "Target no expuso una galería utilizable en el HTML recibido.",
  };
}
