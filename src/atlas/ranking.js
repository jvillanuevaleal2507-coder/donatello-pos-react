...
function isPromotional(result = {}) {
  ...
  return PROMOTION_TERMS.some((term) => text.includes(normalizeText(term)));
}
...
function getTitleHintScore(result = {}) {
  const value = Number(result.titleHintScore ?? result.metadata?.titleHintScore ?? 0);
...
function getTokens(result = {}) {
  ...
  return new Set(text.split(" ").map((token) => token.trim()).filter(
...
function numericCompatibility(a, b) {
  ...
  for (const value of numbersA) if (numbersB.has(value)) return 1;
  return -1;
}

function detectGroup(text, groups) {
  return groups.findIndex((group) => group.some((term) => text.includes(term)));
}
...
function compareStableText(a, b) {
  return deterministicKey(a).localeCompare(deterministicKey(b), "en", { sensitivity: "base" });
}
...
function findStableAnchor(results = []) {
  ...
  const decorated = results.map((result) => ({
    ...
    sourcePriority: getStorePriority(normalizeSource(result.source)),
  }));
...
function productCompatibility(result, anchor) {
  ...
  if (numeric < 0 && titleSimilarity < 0.35) return -0.5;
  return Math.min(1, titleSimilarity + (numeric > 0 ? 0.2 : 0));
}
...
function hasMeasurementImage(result = {}) {
  return Array.isArray(result.images) && result.images.some((image) => {
    ...
    const text = [image?.url, image?.alt, image?.title, image?.variant]
      .filter(Boolean).join(" ").toLowerCase();
    return type === "measurements" || /dimension|dimensions|measurement|measurements|size chart|width|height|depth|length|inch|inches|medidas|dimensiones/.test(text);
  });
}
...
export function chooseBestResult(results = []) {
  ...
  const compatible = pool.filter((result) => result.compatibleWithAnchor);
  const selectedPool = compatible.length ? compatible : pool;
  const selectedIds = new Set(selectedPool.map((result) => result.id));
  const rejected = ranked.filter((result) => !selectedIds.has(result.id));

  return {
    best: selectedPool[0] || null,
    alternatives: selectedPool.slice(1),
    rejected,
  };
}
